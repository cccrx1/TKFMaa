...
- `activity_groups.<id>.cases` 描述各关卡之间的差异；当前启用配置只提供指定关卡。
...
| `mode`                 | 生成器兼容字段；旧自动通关模式为 `regular_auto_clear` |
...
生成器仍识别 `mode: regular_auto_clear`：它会把 `DailyStaminaPresetInnerRouteStart.next` 改为
`RegularActivityStageListDecision`，并把活动组的 `stage.any_selectable` 写入
`RegularActivityPresetStageListMarker`。当前 `active.yaml` 与生成后的关卡选项没有启用这一模式，
不要把兼容分支当作现有任务选项。

自动通关现由独立的[自动战斗任务](../tasks/auto_battle.md)提供，入口是 `AutoBattleStart`，需要手动进入
受支持的常规活动关卡列表；它不读取本指南的活动 Case。扫描、战斗、体力判断与扩展限制统一见该任务文档。
...
- 独立自动战斗需要适配复刻或大型活动时，参见[自动战斗](../tasks/auto_battle.md)，不属于指定关卡 YAML 换期。
