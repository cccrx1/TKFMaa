...
本文档说明向 TKFMaa 提交 Pull Request（PR）时的协作约定。
...
- **优先复用**：新增能力前，先确认项目现有 Pipeline、Agent、MaaFramework 或 MaaCommonAssets 是否已有可复用实现。
...
| 类型              | 示例                       | 适用场景                   |
| ----------------- | -------------------------- | -------------------------- |
| `feat/<name>`     | `feat/stamina-route`       | 新增任务或能力             |
...
- 修改 Schema、资源目录或工作流时，需要说明对 TKFMaa 构建、发布和现有用户配置的影响。
...
- 用户可见流程、活动配置方式或开发约定发生变化时，应同步更新文档。
- 依赖、构建、发布相关改动需要说明动机，并提交对应锁文件变更。
- 如果改动会影响已有任务配置或用户选项，请在 PR 描述中明确说明迁移影响。
