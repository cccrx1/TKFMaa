...
- 2026-07-16：为 360 秒状态分发超时增加一次有上限的应用重启和二次超时停止节点；正常登录主流程
  不变。超时恢复分支已通过静态 Pipeline 校验，仍待通过可控的异常页面或断网场景完成实机触发验证。
- 2026-07-16，MuMuPlayer v5+，官服：从主界面运行修改后的 `GameLoginStart`，正常完成停止应用、
  重新启动、状态分发并返回主界面，新增超时恢复节点未影响主流程。MaaMCP 返回层仍缺少结构化
  TaskDetail，本轮以最终同时识别到“出征”和“调教”确认正常登录回归通过。
...
- 2026-07-19，客户端 `Ver.2.2.1`、MuMuPlayer v5+、官服：从模拟器桌面启动官服后运行
  `GameLoginStart`，标题页按预期由横屏切换为 720×1280 竖屏，最终主界面同时识别到“出征”和
  “调教”。本轮覆盖批量增加交互前 `pre_wait_freezes: 300` 后的正常登录回归，未出现标题页持续动画
  阻塞点击。MaaMCP 返回层仍因缺少结构化输出报校验错误，因此以最终画面确认结果，并在结束后调用
  `stop_pipeline` 清理任务；未取得可用的 TaskDetail。
