...
网络缓冲期间，当前页面的按钮、标题等界面元素可能仍然存在并能被 OCR 或模板识别，
但画面底部会显示缓冲条；此时点击通常没有响应，直接沿识别结果进入下一个节点会造成误操作或流程失败。
缓冲时间取决于网络状况，可能持续一两秒，也可能更久。因此“识别到了页面元素”不能单独作为页面已就绪的依据，
必须先排除缓冲状态，再确认目标页面可以继续操作。缓冲识别存在 OCR 漏判的可能，未识别到缓冲条也不能反过来证明页面已经就绪。
...
- `CommonWaitLoading` 在底部 ROI `[180, 1158, 360, 92]` 内按词干宽松识别加载文字：
  `LOAD` 以及 `CONNECTING` 中的 `ONNEC / ONEC` 片段。兼容大小写、字母间空格、`O/0` 和该片段末尾的 `C/G`
  混淆；不要求后半截 `TING / ING` 完整，也允许开头 `C` 被 OCR 拆到另一文本框。
  规则不限定单词边界，包含上述片段的其他底部文字也会进入等待；按偏向等待的策略处理。
- 加载文字命中时等待 500 毫秒，再由 `[JumpBack]` 回到父节点复查；文字消失后才继续判断其他候选，
  不要求整屏静止。该节点需要显式放在 `next` 的其他页面候选之前，并非自动全局拦截。
  OCR 未读出关键片段时仍可能漏判；没有匹配到加载文字本身不代表页面已经就绪，仍须识别目标页面。
- 缓冲期间即使主界面或业务页面的识别条件仍然命中，也必须把缓冲检测放在这些候选之前；
  不应使用“连续几次未识别到缓冲”作为成功条件。安全完成条件是缓冲处理后再次命中可操作的目标页面，
  必要时允许重复复查或等待至超时，由调用方处理失败，不在缓冲漏判时盲目点击。
- 结束重试节点使用短间隔复查，并允许处理加载和礼包。最后必须识别主界面，不能只因点击发出而结束。
- 主界面首次同时识别到“出征”和“调教”后，`CommonStopOnMainObserve` 会先观察 1 秒，
  再由 `CommonStopOnMainConfirm` 重新优先检查缓冲和礼包，最后才执行停止；这样可以覆盖常见的
  “主体已经回到主界面、底部网络缓冲条仍在显示”的情况。该观察不能替代缓冲识别，缓冲 OCR 漏判时仍以目标页面复查和超时为安全边界。
- 点击前的 300 毫秒稳定等待保留。
